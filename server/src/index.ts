...
async function ensureTable(domain: string): Promise<Table> {
  ...
  if (!tables.includes(tableName)) {
    const table = await db.createTable(tableName, [
      { id: '', url: '', title: '', content: '', image: '', embedding: new Array<number>() }
    ]);
    await table.add([]);
    return table;
  }
...
app.post('/api/query', async (req: Request, res: Response) => {
  try {
    ...
    const context = results.map((r: any, idx: number) => `Snippet ${idx + 1} (url: ${r.url}):\n${r.content}`).join('\n\n');
    ...
    const related = results.map((r: any) => ({ title: r.title, url: r.url, image: r.image })).slice(0, 6);
